<script lang="ts">
  ...
  import { Trash2, Smartphone, Laptop, Tablet, Monitor } from "lucide-svelte";
  ...
  function getDeviceIcon(deviceInfo: string) {
    ...
    if (info.includes("mobile") || info.includes("android") || info.includes("iphone")) {
      ...
      return Tablet;
    } else if (info.includes("windows") || info.includes("mac") || info.includes("linux")) {
      return Laptop;
    }
    return Monitor;
  }
...
<Card.Root>
  <Card.Header>
    ...
    <Card.Description>Kelola sesi aktif dan device Anda.</Card.Description>
  </Card.Header>
  <Card.Content>
    {#if deviceStore.isLoading}
      <div class="flex justify-center p-8">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
        <span class="ml-3 text-muted-foreground">Memuat device...</span>
      </div>
    {:else if deviceStore.devices.length === 0}
      <div class="text-center p-8 text-muted-foreground">
        <Smartphone class="w-12 h-12 mx-auto mb-4 opacity-50" />
        <p>Tidak ada device aktif.</p>
      </div>
    {:else}
        ...
        <Table.Root>
          <Table.Header>
            <Table.Row>
              ...
              <Table.Head class="max-w-[200px]">Device Info</Table.Head>
              <Table.Head>Terakhir Aktif</Table.Head>
            ...
            {#each deviceStore.devices as device}
              ...
              <Table.Row>
                ...
                <Table.Cell class="max-w-[200px]">
                  <div class="font-medium truncate" title={device.device_info || "Unknown Device"}>
